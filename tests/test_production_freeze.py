...
#: Merging main's party-chart commit 2bff422 into the release adds two purely additive
#: files to the freeze closures: normalize.py gains the two chart parsers (74 added, 0
#: removed) and validate.py a field-name tuple (13 added, 0 removed). No existing
#: function changed, production does not import either addition, and the published
#: artifact is byte-identical - so this is recorded as known, intentional drift rather
#: than repaired by re-issuing a freeze or reverting main's commit. The series those
#: parsers read is retrospectively recomputed and is barred from historical-state and
#: production-state use (see data/README.md).
