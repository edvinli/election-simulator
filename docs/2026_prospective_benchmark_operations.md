...
All shares are percentage points on the official national valid-vote denominator. The eight parties are never renormalized to 100%. Point MAE uses each system's actually published central prediction. Threshold Brier events are inclusive `share >= 4.0%` for L, C, KD, and MP. Central interval coverage/width is reported where meaningful. Seat and government-event scores are omitted unless both predictive objects and event semantics are genuinely compatible.
