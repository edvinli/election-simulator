"""Hindcast execution engine for Election Result Layer v2 (percentage-point transfers)."""
...
from .residuals_pool import (
    ChronologicalPPResidualsPool,
    derive_election_layer_v2_seed,
...
def run_election_layer_v2_hindcasts(
    ...
    """Run paired Election Result Layer v2 hindcasts across 2018 and 2022."""
    ...
    for election_date in elections:
        ...
        # 1. Load strict chronological percentage-point training pool
        ...
        for h in sorted(horizons, reverse=True):
            ...
            layer_seed = derive_election_layer_v2_seed(seed, origin_date, h)
            ...
            # 5. Base composition matrix from state_plus_dynamics
            base_clr_matrix = state_clr + sym_deltas
            base_comp_matrix = clr_to_composition_matrix(base_clr_matrix)

            # 6. Sample shared historical election indices for noise variants
            rng = np.random.default_rng(layer_seed)
            sampled_indices = rng.integers(0, k, size=samples)

            # 7. Apply 4 election layer v2 variants
            # 7a. Base
            comp_base = base_comp_matrix
            lambdas_base = np.ones(samples, dtype=float)

            # 7b. PP Bias Only
            comp_bias_only, lambdas_bias_only = apply_batch_simplex_transfer(
                base_comp_matrix, training_pool.mean_bias_pp
            )

            # 7c. PP Noise Only (centered residuals)
            res_noise_mat = training_pool.centered_residuals_matrix[sampled_indices]
            comp_noise_only, lambdas_noise_only = apply_batch_simplex_transfer(
                base_comp_matrix, res_noise_mat
            )

            # 7d. PP Bias Plus Noise (raw residuals, shared index!)
            res_raw_mat = training_pool.residuals_matrix[sampled_indices]
            comp_bias_plus_noise, lambdas_bias_plus_noise = apply_batch_simplex_transfer(
                base_comp_matrix, res_raw_mat
            )

            variant_matrices = {
                "base": (comp_base, lambdas_base),
                "pp_bias_only": (comp_bias_only, lambdas_bias_only),
                "pp_noise_only": (comp_noise_only, lambdas_noise_only),
                "pp_bias_plus_noise": (comp_bias_plus_noise, lambdas_bias_plus_noise),
            }

            cases_evaluated_count += 1

            for var_id in variants:
                samples_mat, lam_arr = variant_matrices[var_id]
    ...
    def summarize_group(df_sub: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            ...
            "fraction_lambda_lt_0_99": float(df_sub["fraction_lambda_lt_0_99"].mean()),
    ...
    for v in variants:
        ...
        for e_date in sorted(df_cases["election_date"].unique()):
            ...
            if not sub_ev.empty:
                ev_dict = summarize_group(sub_ev)
                ev_dict["variant"] = v
        ...
        for h in horizons:
            ...
            if not sub_hv.empty:
                hv_dict = summarize_group(sub_hv)
                hv_dict["variant"] = v
    ...
    cases_csv_path = out_dir / "election_layer_v2_cases_2018_2022.csv"
    json_summary_path = out_dir / "election_layer_v2_summary_2018_2022.json"
    election_csv_path = out_dir / "election_layer_v2_by_election_model.csv"
    horizon_csv_path = out_dir / "election_layer_v2_by_horizon_model.csv"
    party_csv_path = out_dir / "election_layer_v2_by_party_model.csv"
